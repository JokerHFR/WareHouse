...
public class PurFrame extends JFrame implements ActionListener{
	/**
	 * 
	 */
    ...
    private Date date;
	public PurFrame(Supplier supplier,List<Product> product){
		...
		this.setTitle("送货订单");
 		...
 		this.jl2=new JLabel("     进货地址："+this.supplier.getSuppliercomAddress());
 		...
 		for(int i=0;i<this.product.size();i++){
 			...
 			JLabel jl3=new JLabel("总价:"+this.product.get(i).getQuantity()*this.product.get(i).getSuggestBuyPrice());
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==this.jb1){
			...
			this.productBL.productupdatepur(product, date);
			this.supplierBL.updatesupplierdate(supplier, date);
			JOptionPane.showMessageDialog(this, "进货订单生成成功","提示消息",JOptionPane.INFORMATION_MESSAGE);
			this.dispose();
		}
